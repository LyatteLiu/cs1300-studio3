...
const updatePage = async () => {
  ...
  const filteredNames = filtered.map((fruit) => {return " " + fruit.name});
  console.log(filtered);
  // TODO: Create a new HTML element to display your data
  const newElement = document.createElement('div');
  newElement.innerHTML = filteredNames;
...
const addFruitArray = () => {
  ...
  const existingElement = document.getElementById('gallary-container');
  existingElement.append(newElement);
}
